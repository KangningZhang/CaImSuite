...
filename = "E:/Kangning Zhang/CaImSuite/Dataset/1P NAOMi1p v0/mov_wo_bg_600_600_150_depth_100_1_512.tiff"
print(filename)
...
def histogram_cluster_frame(feature_maps, n_bins=50, n_components=10):
    """
    Perform pixelwise histogram-based clustering using full feature vectors
    ...
    hist_range = (features.min(), features.max())
    pixel_histograms = np.zeros((H*W, n_bins))
    bin_edges = np.linspace(hist_range[0], hist_range[1], n_bins+1)
    
    # Calculate histogram for each pixel's feature vector
    for i in range(H*W):
        hist, _ = np.histogram(features[i], bins=bin_edges, density=True)
        pixel_histograms[i] = hist
    
    # Apply PCA to reduce dimensionality of histogram patterns
    pca = PCA(n_components=min(n_components, n_bins))
    reduced_histograms = pca.fit_transform(pixel_histograms)
    
    # Add small random noise to prevent singular matrices
    noise = np.random.normal(0, 1e-10, reduced_histograms.shape)
    reduced_histograms += noise
    
    # Estimate density of reduced histogram patterns
    kde = gaussian_kde(reduced_histograms.T)
    density_scores = kde(reduced_histograms.T)
    
    # Find natural threshold in density distribution
    x_grid = np.linspace(density_scores.min(), density_scores.max(), 200)
    density = gaussian_kde(density_scores)(x_grid)
    peaks, _ = find_peaks(-density)  # Finding valleys
    ...
    if len(peaks) > 0:
        ...
        threshold = np.median(density_scores)
    
    # Create binary segmentation
    binary_seg = (density_scores > threshold).reshape(H, W)
    
    return binary_seg.astype(float)

# Replace simple thresholding with pixelwise histogram clustering
print("Performing pixelwise histogram-based clustering...")
...
for frame_idx in tqdm(range(sparse_maps.shape[0])):
    ...
    seg[frame_idx] = histogram_cluster_frame(frame_features)
